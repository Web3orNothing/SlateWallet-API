...
function getProtocolAddressFromName(protocolName) {
  ...
  const protocolNamesToAddresses = {
    ...
    aave: '0xF1Cd4193bbc1aD4a23E833170f49d60f3D35a621', // '0x7fc66500c84a76ad7e9c93437bfc5ac33e2ddae9',
...
app.post('/protocol', async (req, res) => {
  try {
    const { name, action, spender, receiver, inputAmount, inputToken, outputAmount, outputToken, chainName } = req.body;

    const protocolAddress = getProtocolAddressFromName(name);
    const protocolData = await get_contract_data(protocolAddress);
    var data = '';
    ...
    const transactionDetails = {
      ...
      gasPrice: 355250,
      value: amount,
      data: data, // filled with data generated by the function call encoding above
      nonce: 'nonce',
    };
